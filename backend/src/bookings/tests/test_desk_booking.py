"""
Tests for desk booking creation, conflicts, cancellation, and concurrency.
"""
...
class TestDeskBookingService:
    ...
    def test_concurrent_booking_lock_contention(self, user, desk, booking_date):
        mock_queryset = MagicMock()
        mock_queryset.filter.return_value = mock_queryset
        mock_queryset.get.side_effect = OperationalError('could not obtain lock')

        with patch(
            'bookings.services.Desk.objects.select_for_update',
            return_value=mock_queryset,
        ):
            with pytest.raises(DeskAlreadyBooked, match='currently being booked'):
                create_desk_booking(user, desk.id, booking_date)
...
class TestDeskBookingAPI:
    ...
    @patch('bookings.views.create_desk_booking')
    def test_concurrent_booking_attempts_via_api(
        self, mock_create_desk_booking, auth_client, desk, booking_date
    ):
        mock_create_desk_booking.side_effect = DeskAlreadyBooked(

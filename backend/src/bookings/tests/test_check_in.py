...
from bookings.services import check_in_booking, create_desk_booking, create_room_booking
...
def check_in_url(booking_id):
    return f'{BOOKINGS_URL}{booking_id}/check_in'
...
class TestCheckInService:
    def test_desk_check_in_success(self, user, desk):
        ...
        with patch('bookings.services.timezone.now', return_value=frozen_now):
            with patch('bookings.services.timezone.localdate', return_value=date(2026, 7, 10)):
    ...
    def test_room_check_in_success(self, user, room):
        ...
        frozen_now = datetime(2026, 7, 10, 14, 15, tzinfo=timezone.utc)
    ...
    def test_check_in_rejects_wrong_date(self, user, desk):
        desk_booking = create_desk_booking(user, desk.id, date(2026, 7, 10))
        frozen_now = datetime(2026, 7, 11, 9, 0, tzinfo=timezone.utc)

        with patch('bookings.services.timezone.now', return_value=frozen_now):
            with patch('bookings.services.timezone.localdate', return_value=date(2026, 7, 11)):
                with pytest.raises(CheckInNotAllowed, match='booking day'):
                    check_in_booking(user, desk_booking)

    def test_check_in_rejects_after_desk_cutoff(self, user, desk):
        desk_booking = create_desk_booking(user, desk.id, date(2026, 7, 10))
        frozen_now = datetime(2026, 7, 10, 10, 30, tzinfo=timezone.utc)

        with patch('bookings.services.timezone.now', return_value=frozen_now):
            with patch('bookings.services.timezone.localdate', return_value=date(2026, 7, 10)):
                with pytest.raises(CheckInNotAllowed, match='closed'):
                    check_in_booking(user, desk_booking)
...
class TestCheckInAPI:
    def test_desk_check_in_endpoint(self, auth_client, desk):
        ...
        with patch('bookings.services.timezone.now', return_value=frozen_now):
            with patch('bookings.services.timezone.localdate', return_value=date(2026, 7, 10)):
                response = auth_client.post(check_in_url(desk_booking.id), format='json')
    ...
    def test_room_check_in_endpoint(self, auth_client, room):
        ...
        with patch('bookings.services.timezone.now', return_value=frozen_now):
            with patch('bookings.services.timezone.localdate', return_value=date(2026, 7, 10)):
                response = auth_client.post(check_in_url(room_booking.id), format='json')
    ...
    def test_check_in_requires_authentication(self, api_client, user, desk):
        desk_booking = create_desk_booking(user, desk.id, date(2026, 7, 10))
        response = api_client.post(check_in_url(desk_booking.id), format='json')
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_check_in_other_users_booking_returns_403(
        self, other_auth_client, user, desk
    ):
        desk_booking = create_desk_booking(user, desk.id, date(2026, 7, 10))
        response = other_auth_client.post(check_in_url(desk_booking.id), format='json')
    ...
    def test_check_in_wrong_date_returns_400(self, auth_client, desk):
        user = User.objects.get(email='checkin@example.com')
        desk_booking = create_desk_booking(user, desk.id, date(2026, 7, 10))
        frozen_now = datetime(2026, 7, 11, 9, 0, tzinfo=timezone.utc)
